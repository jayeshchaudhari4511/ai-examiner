...
const Evaluate = ({ setLoading }) => {
  ...
  const loadUsers = async () => {
    try {
      const [teachersRes, studentsRes] = await Promise.all([getAllTeachers(), getAllStudents()]);
      const loadedTeachers = teachersRes.teachers || [];
      setTeachers(loadedTeachers);
      setStudents(studentsRes.students || []);

      if (role === 'teacher' && user?.email) {
        const matchedTeacher = loadedTeachers.find(
          (teacher) => (teacher.email || '').toLowerCase() === user.email.toLowerCase()
        );
        if (matchedTeacher?._id) {
          setSelectedTeacher(matchedTeacher._id);
        }
      }
    } catch (err) {
  ...
  const handleEvaluate = async () => {
    ...
    try {
      ...
      const teacherIdToUse = role === 'teacher' ? selectedTeacher || user?._id || '' : selectedTeacher;
      if (teacherIdToUse) {
        formData.append('teacher_id', teacherIdToUse);
      }
      ...
      <div className="evaluate-container">
        ...
        {step === 1 && (
          <div className="step-content">
            ...
            <button className="btn btn-primary btn-lg" onClick={() => setStep(2)} disabled={!modelAnswerText}>
              Next <span>→</span>
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="step-content">
            ...
            <div className="form-section">
              <h3>{role === 'teacher' ? 'Student' : 'Teacher & Student'}</h3>
            ...
            <div className="button-group">
              ...
              <button className="btn btn-primary btn-lg" onClick={handleEvaluate} disabled={!studentFile || !maxMarks}>
                Evaluate <span>→</span>
              </button>
            </div>
          </div>
